...
export async function GET(request: NextRequest) {
  ...
  const apiUrl = process.env.OPENWEATHER_FORECAST_URL;

  if (!apiKey || !apiUrl) {
    return NextResponse.json({ message: 'Forecast API is not configured' }, { status: 500 });
  }
  ...
  return NextResponse.json(groupForecastByDay(data.list));
}
